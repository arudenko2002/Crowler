...
public class SpotifyChartsViralProcess implements SpotifyService{
SpotifyHttpService spotifyHttpService;
	...
	public void run(String[] args) throws ClientProtocolException, URISyntaxException, IOException, AmazonServiceException, AmazonClientException, InterruptedException {
		SpotifyChartsViralProcess scp = new SpotifyChartsViralProcess();
		...
		try {
			...
			File regionalPath = FileUtil.getLocalFilePath(CHARTS_VIRAL_FILE,dateString);
			List<String> countries =scp.getAvailableMarkets(CHARTS_VIRAL_URL);
			System.out.println(" contries length >> " + countries.size());
			System.out.println(" regional charts started ");
			scp.getReginalspotifyCharts(countries,regionalPath,latest,CHARTS_VIRAL_TYPE);
			...
			}else{System.out.println(latest+ " Date is not available " );
	...
	private List<String> getAvailableMarkets(String uri) {
		...
		private List<String> getAvailableDates() {
			...
			try {
				...
				HttpResponse response = getspotifyHttpService().doWithGet(CHARTS_VIRAL_DATES_URL);
				int responseCode = response.getStatusLine().getStatusCode();
				if(responseCode == HttpStatus.SC_OK){
					String stringResponse = EntityUtils.toString(response.getEntity());
